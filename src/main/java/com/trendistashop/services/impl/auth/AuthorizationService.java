...
import com.trendistashop.repositories.auth.PermissionRepository;
...
public class AuthorizationService implements IAuthorizationService {
        ...
        public TypeResponse<RoleDTO> getRoleByName(String name) {
                ...
                if (role == null) {
                        ResponseHelper.notFound(ResponseMessage.ROLE_NOT_FOUND);
                }
                return ResponseHelper.ok(modelMapper.map(role, RoleDTO.class), ResponseMessage.FETCH_SUCCESS);
        }
        ...
        public TypeResponse<List<RoleDTO>> getAllRoles() {
                ...
                return ResponseHelper.ok(roleDTOS, ResponseMessage.FETCH_SUCCESS);
        }
        ...
        public RoleDTO removePermissionsFromRole(UUID roleId, Set<UUID> permissionIds) {
                ...
                role.getPermissions().removeIf(permission -> permissionIds.contains(permission.getId()) // cos hay kh
                );
                // Save updated role
                RoleEntity updatedRole = roleRepository.save(role);
